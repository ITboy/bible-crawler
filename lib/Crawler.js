...
class Crawler extends EventEmitter {
  ...
  crawlBible(bible) {
    this.request.get(bible.indexUrl).then(res => {
      ...
      // crawl testament
      const testaments = [{ isNew: true, testamentUrl: newTestamentUrl, bible: bibleData }, { isNew: false, testamentUrl: oldTestamentUrl, bible: bibleData }];

      return new Promise((resolve, reject) => async.each(testaments, (item, cb) => this.crawlTestament(item, cb), error => {
        if (error) {
          reject(error);
        } else {
          resolve(bibleData);
  ...
  crawlTestament(testament, callback) {
    this.request.get(testament.testamentUrl).then(res => {
      ...
      return new Promise((resolve, reject) => async.each(testamentData.books, (item, cb) => this.crawlBook(item, cb), error => {
        if (error) {
          reject(error);
        } else {
  ...
  crawlBook(book, callback) {
    this.request.get(book.bookUrl).then(res => {
      ...
      return new Promise((resolve, reject) => async.each(bookData.chapters, (item, cb) => this.crawlChapter(item, cb), error => {
        if (error) {
          reject(error);
        } else {
          resolve(bookData);
  ...
  crawlChapter(chapter, callback) {
    this.request.get(chapter.chapterUrl).then(res => {
      ...
      return new Promise((resolve, reject) => async.each(chapterData.sections, (item, cb) => this.crawlSection(item, cb), error => {
        if (error) {
          reject(error);
        } else {
          resolve(chapterData);
