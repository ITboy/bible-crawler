...
class CachedSuperAgent {
  ...
  resume() {
    // 处理等待队列的请求
    while (this.canRequest() && this.waitQueue.length > 0) {
  ...
  get(url) {
    if (this.canRequest()) {
      const requestPromise = request.get(url).charset(this.charset).timeout({ response: this.timeout, deadline: this.deadline }).retry(this.maxRetriedTimes).then(res => {
