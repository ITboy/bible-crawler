...
class CrawlerHehe extends Crawler {
  ...
  parseBible() {
    ...
    return _extends({}, this.bible, { newTestamentUrl, oldTestamentUrl });
  }
  ...
  parseTestament(res, testament) {
    ...
    const { testamentUrl } = testament,
          testamentData = _objectWithoutProperties(testament, ['testamentUrl']);
    let testamentName;
    ...
    const $testament = $('div.tm.cn').filter((i, element) => {
      ...
      const matched = isNew && name === NEW_TESTAMENT_NAME || !isNew && name === OLD_TESTAMENT_NAME;
  ...
  parseBook(res, book) {
    ...
    const chapters = [];
    const { bookUrl } = book,
          bookData = _objectWithoutProperties(book, ['bookUrl']);
  ...
  parseChapter(res, chapter) {
    const { chapterUrl } = chapter,
          chapterData = _objectWithoutProperties(chapter, ['chapterUrl']);
    const $ = cheerio.load(res.text);
